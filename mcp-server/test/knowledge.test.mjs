...
test('listEffects reads the radar scan manifest', async () => {
  ...
  assert.equal(effects[2].id, 'polyline-flow')
  assert.equal(effects[3].id, 'fly-line')
  assert.equal(effects[4].id, 'pipe-flow')
  assert.equal(effects[5].id, 'light-wall')
  assert.equal(effects[6].id, 'scan-cone')
  assert.equal(effects[7].id, 'shield-dome')
  assert.equal(effects[8].id, 'temperature-field')
  assert.equal(effects[9].id, 'fire-billboard')
